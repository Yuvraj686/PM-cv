...
from schemas.schemas import ChannelCreate, ChannelOut, MessageOut
...
async def _channel_room_id(channel: Channel, db: AsyncSession) -> str:
    ...
    if channel.type == "dm":
        members = await db.execute(select(ChannelMember).where(ChannelMember.channel_id == channel.id))
...
async def list_channels(
    ...
    if project_id:
        result = await db.execute(
            ...
            .where(Channel.project_id == project_id, ChannelMember.user_id == current_user.id)
        )
    else:
        result = await db.execute(
            select(Channel)
...
async def create_channel(payload: ChannelCreate, current_user: CurrentUser, db: DB):
    if payload.type == "dm":
        raise HTTPException(status_code=400, detail="Use the DM endpoint to create direct messages")
    channel = Channel(
...
async def get_or_create_dm(other_user_id: str, current_user: CurrentUser, db: DB):
    ...
    for ch in my_channels.scalars().all():
        members = await db.execute(select(ChannelMember).where(ChannelMember.channel_id == ch.id))
        ...
        while True:
            ...
            if event_type == "chat_message":
                ...
                await manager.broadcast_to_room(
                    room_id,
                    {"type": "typing", "user_id": user_id, "is_typing": data.get("is_typing", True)},
...
async def _handle_chat_message(room_id: str, sender_id: str, data: dict):
    ...
    async with AsyncSessionLocal() as db:
        ...
        if channel and channel.type == "dm":
            members = await db.execute(select(ChannelMember).where(ChannelMember.channel_id == channel_id))
            member_ids = [str(m.user_id) for m in members.scalars().all()]
