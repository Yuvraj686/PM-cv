...
from schemas.auth import (
    PhoneSendOTPRequest, PhoneVerifyOTPRequest, 
    PhoneResendOTPRequest, TokenResponse
)
...
async def verify_otp(
    ...
    if user.phone_otp_locked_until and user.phone_otp_locked_until > datetime.utcnow():
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Too many attempts. Try again after {lock_until}."
        )
    ...
    if user.phone_otp != req.otp:
        ...
        if user.phone_otp_attempts >= 5:
            ...
            await db.commit()
            raise HTTPException(status_code=400, detail="Too many wrong attempts. Locked for 10 minutes.")
    ...
    access_token = create_access_token({"sub": str(user.id), "phone": user.phone_number})
    refresh_token = create_refresh_token({"sub": str(user.id), "phone": user.phone_number})
...
async def resend_otp(
    ...
    if user.phone_verified:
        raise HTTPException(status_code=400, detail="Already verified. Please log in.")
    
    # Check if trying to resend too quickly (within 60 seconds)
    if user.phone_otp_exp:
        time_since_otp = datetime.utcnow() - (user.phone_otp_exp - timedelta(minutes=10))
