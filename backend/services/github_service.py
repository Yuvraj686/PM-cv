...
async def fetch_github_issues(repo_url: str) -> list[dict]:
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.get(api_url, headers=headers, params={"state": "all"})
...
async def sync_repo_issues(project_id: str, repo_url: str, db: AsyncSession | None = None):
...
async def _sync_logic(project_id: str, issues: list[dict], db: AsyncSession):
    ...
    result = await db.execute(
        select(Task).where(Task.project_id == project_id, Task.external_source == "github")
    )
